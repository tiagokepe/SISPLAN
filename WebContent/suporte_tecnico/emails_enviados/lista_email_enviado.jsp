...
<f:view>
	...
	<admin:caminho titulo="Consultar E-mails Enviados" />

	<h:form id="form">
		<table class="formulario" align="center" width="400">
			<caption class="listagem">Consulta de E-mails Enviados</caption>
			<tr>
				<td width="4%"><h:selectBooleanCheckbox value="#{emailEnvidadoMBean.buscaDestinatario}" id="selectDestinatario" styleClass="noborder"/></td>
				<th style="text-align: left;" width="15%">Destinatário(s)</th>
				<td><h:inputText
					value="#{emailEnvidadoMBean.objFiltro.paraNome}" id="inputDest" onclick="$('form:selectDestinatario').checked = true" /></td>
			</tr>
			<tr>
				<td><h:selectBooleanCheckbox value="#{emailEnvidadoMBean.buscaEmail}" id="selectEmail" styleClass="noborder"/></td>
				<th nowrap style="text-align: left;">Email do Destinatário:</th>
				<td><h:inputText
					value="#{emailEnvidadoMBean.objFiltro.paraEmail}" id="inputEmail" onclick="$('form:selectEmail').checked = true" /></td>
			</tr>
			<tr>
				<td><h:selectBooleanCheckbox value="#{emailEnvidadoMBean.buscaAssunto}" id="selectAssunto" styleClass="noborder"/></td>
				<th style="text-align: left;">Assunto:</th>
				<td><h:inputText
					value="#{emailEnvidadoMBean.objFiltro.assunto}" id="inputAssunto" onclick="$('form:selectAssunto').checked = true" /></td>
			</tr>
			<tr>
				<td><h:selectBooleanCheckbox value="#{emailEnvidadoMBean.buscaDataInicio}" id="selectDataInicio" styleClass="noborder"/></td>
				<th style="text-align: left;">Início:</th>
				<td><t:inputCalendar
					value="#{emailEnvidadoMBean.objFiltro.dataInicio}"
			...
			<tr>
				<td><h:selectBooleanCheckbox value="#{emailEnvidadoMBean.buscaDataFim}"  id="selectDataFim" styleClass="noborder"/></td>
				<th style="text-align: left;">Fim:</th>
				<td><t:inputCalendar
					value="#{emailEnvidadoMBean.objFiltro.dataFim}"
					popupDateFormat="dd/MM/yyyy" size="10" maxlength="10"
			...
			<tfoot>
				<tr>
					<td colspan="3"><h:commandButton value="Listar" id="listar"
						action="#{ emailEnvidadoMBean.listar }" /><h:commandButton id="cancelar"
						value="Cancelar" action="#{ emailEnvidadoMBean.cancelar }"
						onclick="" /></td>
				</tr>
			</tfoot>
		</table>
		<br />
		<br />
		<t:htmlTag value="div" styleClass="infoAltRem" rendered="#{emailEnvidadoMBean.colecao.rowCount > 0}">
			<h:graphicImage value="/img/geral/lupa.gif" />: Visualizar E-mail
		</t:htmlTag>
		<h:dataTable width="100%" value="#{emailEnvidadoMBean.colecao}"
			var="item" id="TableEmails" styleClass="listagem"
			rowClasses="linhaImpar, linhaPar" rendered="#{emailEnvidadoMBean.colecao.rowCount > 0}">
			...
			<t:column>
				<h:commandLink title="Visualizar E-mail" action="#{emailEnvidadoMBean.verEmailEnviado}" id="cmdlinkVerEmail">
		...
		<c:if test="${emailEnvidadoMBean.colecao.rowCount > 0}">
			<br/>
